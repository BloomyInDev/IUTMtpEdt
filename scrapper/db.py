...
class Db:
    ...
    def __init__(self) -> None:
        self.db = connect(
            host="127.0.0.1", port=3306, user="root", password="ThisPasswordIsntSecure", database="iutmtpedt", charset="utf8mb4", collation="utf8mb4_unicode_ci"
        )
        self.table = ""
...
class Professor(Db):
    ...
    def add(self, first_name: str, last_name: str) -> None:
        self.cur.execute(f"SELECT COUNT(*) FROM {self.table} WHERE prenom = %s AND nom = %s", (first_name,last_name,))
    ...
    def get(self, first_name: str | None = None, last_name: str | None = None) -> list[tuple[int,str,str]]:
        ...
        if (first_name != None and last_name != None):
            self.cur.execute(f"SELECT * FROM {self.table} WHERE prenom = %s AND nom = %s", (first_name, last_name))
            ...
            self.cur.execute(f"SELECT * FROM {self.table} WHERE prenom = %s", (first_name,))
            result = self.cur.fetchall()
...
class Event(Db):
    ...
    def add(self, name:str, timeStart:int, timeEnd:int, profs:list[tuple[str,str]], groups: list[str]) -> None:
        """
